...
class TicTacToe extends SmartContract {
  ...
  @state(Field) nextPlayer: State<Field>; // 1 -> player 1 | 2 -> player 2
  ...
  @method async play(pubkey: PublicKey, player: Field, x: Field, y: Field) {
    ...
    const board = Circuit.witness(
      ...
      () => {
        ...
        let res = [];
        let power_of_3 = BigInt(1);
        for (let i = 0; i < 3; i++) {
          let row = [];
          for (let j = 0; j < 3; j++) {
            const cell = (board_bigint / power_of_3) % BigInt(3);
            power_of_3 *= BigInt(3);
    ...
    let won = new Bool(false);
    {
      ...
      for (let i = 0; i < 3; i++) {
        const played = updated_board[i][0]
          .equals(Field.one)
          .or(updated_board[i][0].equals(new Field(2)));
      ...
      for (let i = 0; i < 3; i++) {
        const played = updated_board[0][i]
          .equals(Field.one)
          .or(updated_board[0][i].equals(new Field(2)));
      ...
      const played = updated_board[1][1]
        .equals(Field.one)
        .or(updated_board[1][1].equals(new Field(2)));
...
export async function main() {
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player2.toPublicKey(), two, new Field(1), new Field(0))
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player1.toPublicKey(), Field.one, new Field(1), new Field(1))
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
  ...
  await Mina.transaction(player2, async () => {
    snappInstance
      .play(player2.toPublicKey(), two, new Field(2), new Field(1))
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
  ...
  await Mina.transaction(player1, async () => {
    snappInstance
      .play(player1.toPublicKey(), Field.one, new Field(2), new Field(2))
      .catch((e) => console.log(e));
  })
    .send()
    .wait();
