...
class Board implements AsFieldElements<Board> {
  ...
  static toFields(x: Board): Field[] {
    return [x.serialize()];
  }

  static ofFields(x: Field[]): Board {
    return new Board(x[0]);
  }
...
class TicTacToe extends SmartContract {
  ...
  constructor(
    ...
    this.board = State.init(new Board(Field.zero));
  ...
  @method async play(
    ...
    let board = await this.board.get();
